...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (!(await validSignature(request, orderId.toLowerCase()))) {
    ...
    if (!orderResponse.ok) {
      throw new Error(
        order?.message ?? `Mercado Pago respondeu ${orderResponse.status}`,
      );
    }
    ...
    if (paid && order.external_reference) {
      ...
      const { error } = await admin.rpc("mark_subscription_invoice_paid", {
        target_external_reference: order.external_reference,
    ...
    // Retorna 500 para que o provedor possa reenviar a notificacao em falhas
    // transitorias. A rotina subscription-maintenance continua sendo a segunda
    // camada de reconciliacao caso o webhook demore.
