...
import {
  BillingPanel,
  SubscriptionBlocked,
} from "../../billing/components/BillingPanel";
import {
  loadBilling,
  loadCurrentRole,
  subscriptionAllowsAccess,
} from "../../billing/services/billingApi";
...
export function AuthGate({ children }: { children: ReactNode }) {
  ...
  // Re-check subscription status in real-time so unblocking after payment takes effect immediately
  useEffect(() => {
    if (!user) return;
    const channel = supabase
      .channel("authgate-subscription")
      .on(
        "postgres_changes",
        { event: "UPDATE", schema: "public", table: "app_subscription" },
        async () => {
  ...
  // Poll every 5s when blocked (detect payment/unblock), every 30s when active (detect blocking)
  useEffect(() => {
    if (!user || !subscription) return;
    const isBlocked = !subscriptionAllowsAccess(subscription);
    const interval = setInterval(
      async () => {
        const billing = await loadBilling().catch(() => null);
        if (billing) setSubscription(billing.subscription);
      },
      isBlocked ? 5_000 : 30_000,
    );
      ...
      <main className="database-error-page">
        <section className="login-card">
          <div className="login-error">
            {accessError || "Conta sem permissão configurada"}
          </div>
          <button
